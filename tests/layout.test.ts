 ...
 */
import { refineLayout, findComponents, LayoutNode, LayoutEdge } from '../src/graph/layout';
import {
  generateVaultGraph,
  fcoseSeed,
  gridSnapSpread,
...
// Twice the entity collision radius, less a hair of float slack: the
// separation the spacing pass guarantees, which is also the label budget
// (labels ellipsize at 80px)
const LABEL_BUDGET = 78;
// Node-level visibility: comfortably under the label budget, so this fails
// only when nodes are genuinely stacked
const VISIBILITY_THRESHOLD = 54;
...
function measure(v: Variant, graph: VaultGraph, components: number[]): Metrics {
  ...
  return {
    visibility: visibility(v.nodes, VISIBILITY_THRESHOLD),
    labelRoom: visibility(v.nodes, LABEL_BUDGET),
    edgeMean: stats.mean,
    edgeP95: stats.p95,
    np: neighborhoodPreservation(v.nodes, graph.edges),
    separation: clusterSeparation(v.nodes, graph.clusterOf, mulberry32(1234)),
    overlap: componentBoxesOverlap(v.nodes, components, LABEL_BUDGET),
    ms: v.ms,
  };
}
...
async function main() {
  ...
  check('refined: >=95% of nodes keep the 80px label budget', fine.labelRoom >= 0.95, `${(fine.labelRoom * 100).toFixed(1)}%`);
  ...
  const spacedRoom = visibility(spaced, LABEL_BUDGET);
  ...
  check('spacing-only: >=95% of nodes keep the label budget', spacedRoom >= 0.95, `${(spacedRoom * 100).toFixed(1)}%`);
